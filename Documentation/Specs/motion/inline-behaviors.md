...
**Not present in design-reference.html.** The `--rotateX`/`--rotateY` CSS variables and tilt `mousemove` handler are not defined in this file. This behavior may exist in a separate file (e.g., animation.html).

---

## 9. Cursor Spotlight

**Not present in design-reference.html.** The `--mouse-x`/`--mouse-y` CSS variables for cursor spotlight are not defined in this file. This behavior may exist in a separate file.

---

## 10. Magnetic Button

**Trigger**: `mousemove` on `#magnetic-zone`, `mouseleave` on `#magnetic-zone`.

**DOM targets**: `#magnetic-zone` (hover area), `#magnetic-btn` (the button that moves).

**Algorithm**:
1. Check `prefersReduced` -- if true, skip setup entirely
2. Set `magneticZone.style.padding = '60px'` (expand hit area)
3. On `mousemove`:
   - Get button center: `btnCx = rect.left + rect.width / 2`, `btnCy = rect.top + rect.height / 2`
   - Calculate distance from cursor to button center: `dx`, `dy`, `dist = Math.sqrt(dx*dx + dy*dy)`
   - If `dist < 120` (within 120px radius): apply pull transform
   - Pull strength: `0.3`
   - Set `transform: translate(dx * 0.3, dy * 0.3)` with `transition: transform 0.2s cubic-bezier(0.22,1,0.36,1)`
4. On `mouseleave`:
   - Reset `transform: translate(0, 0)` with `transition: transform 0.4s cubic-bezier(0.34,1.56,0.64,1)` (bouncy return)

**Parameters**:
- Activation radius: `120px`
- Pull strength: `0.3`
- Pull transition: `0.2s cubic-bezier(0.22, 1, 0.36, 1)` (ease-smooth)
- Reset transition: `0.4s cubic-bezier(0.34, 1.56, 0.64, 1)` (ease-bounce)

**Animation approach**: Direct `style.transform` and `style.transition` manipulation.

**`prefers-reduced-motion` handling**: Entire setup is skipped when `prefersReduced` is true.

---

## 11. Skeleton Loading Toggle
...
## 14. Spring Physics Demos

**Trigger**: Click on `#play-springs` button.

**DOM targets**: 3 box elements: `#spring-snappy`, `#spring-standard`, `#spring-gentle`.

**Algorithm**:
1. For each box (indexed `i = 0, 1, 2`):
   - Reset to `scale(0.5) translateY(20px)`, `opacity: 0`, `transition: none`
   - After `50ms`: apply transition and animate to `scale(1) translateY(0)`, `opacity: 1`

**3 configurations**:

| Box | Duration | Easing |
|---|---|---|
| `spring-snappy` (i=0) | `200ms` | `cubic-bezier(0.34, 1.56, 0.64, 1)` (ease-bounce) |
| `spring-standard` (i=1) | `300ms` | `cubic-bezier(0.22, 1, 0.36, 1)` (ease-smooth) |
| `spring-gentle` (i=2) | `400ms` | `cubic-bezier(0.16, 1, 0.3, 1)` (ease-out) |
...
### 28. Token Architecture Visualization
